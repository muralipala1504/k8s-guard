...
# Import history and slack modules
...
class K8sClient:
    ...
    def delete_pod(self, namespace, name):
        try:
            ...
            send_slack_alert("delete", "pod", name, "success", "Pod deleted by user")
            return True
        except Exception as e:
    ...
    def auto_heal_pods(self, namespace="default"):
        ...
        for pod in pods:
            ...
            if needs_heal:
                ...
                send_slack_alert("delete", "pod", status["name"], "success", heal_reason)
                actions.append({
        ...
        for node in nodes:
            if "NotReady" in node["status"]:
                ...
                try:
                    ...
                    send_slack_alert("uncordon", "node", node["name"], "success", "SchedulingDisabled")
                    actions.append({
                        "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                        "node": node["name"],
                        "action": "uncordoned",
    ...
    def auto_scale_deployments(self, namespace="default"):
        ...
        for deployment in deployments:
            ...
            if total_pods > 0 and failing_pods / total_pods > 0.5 and replicas > 1:
                ...
                send_slack_alert("scale", "deployment", name, "success", f"{replicas}->{new_replicas}")
                actions.append({
                    "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "deployment": name,
                    "action": "scaled_down",
                ...
                send_slack_alert("scale", "deployment", name, "success", "0->1")
                actions.append({
                    "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    "deployment": name,
                    "action": "scaled_up",
